...
export const getProducts = (products, type, limit) => {
    if (type === "featured" || type === "tending" || type === "specials") {
        ...
        const filteredProducts = products.sort((a, b) => b.ratings - a.ratings);

        return filteredProducts.slice(0, limit < 0 ? filteredProducts.length : limit ? limit : filteredProducts.length);

    } else if (type === "best-seller" || type === "best_seller" || type === "popular") {
        const filteredProducts = products.sort((a, b) => b.totalSale - a.totalSale);

        return filteredProducts.slice(0, limit < 0 ? filteredProducts.length : limit ? limit : filteredProducts.length);

    } else if (type === "sale" || type === "deal") {
        const filteredProducts = products.filter(product => product.discount !== null || false || undefined || product.discount > 0);

        return filteredProducts.slice(0, limit < 0 ? filteredProducts.length : limit ? limit : filteredProducts.length);

    } else {
        return products.slice(0, limit < 0 ? products.length : limit ? limit : products.length);
    }
}
...
export const getProductUniqueSizes = (product) => {
    ...
    const sizes = variations ? flatDeep(lists).map(size => size.name) : [product.size];
...
export const getProductColors = (product) => {
    ...
    return variations ? colors.map(color => color) : [{
...
export const getProductsUniqueColors = (products) => {
    ...
    flatDeep(colors).forEach((el) => {
        const isInArr = uniqueColors.findIndex((item) => el.name === item?.name);
        if (isInArr < 0) {
...
export const getProductUniqueMaterials = (product) => {
    ...
    const materials = variations ? flatDeep(lists).map(material => material.name) : [product.material.name];
